...
jni_linux_vkextmemory(void, nLoadFunctions)(JNIEnv* env, jobject) {
...
jni_linux_vkextmemory(jlongArray, nCreateVk)(JNIEnv* env, jobject) {
...
jni_linux_vkextmemory(jlongArray, nCreateExternalImage)(JNIEnv* env, jobject, jlong _device, jlong _physicalDevice, jint width, jint height) {
    ...
    //_vkBindImageMemory(device, textureImage, textureImageMemory, 0);

    // Export memory buffer to file descriptor
    ...
    int fd1;
    _vkGetMemoryFdKHR(device, &getFdInfo, &fd1);

    int fd2;
    _vkGetMemoryFdKHR(device, &getFdInfo, &fd2);
...
jni_linux_vkextmemory(void, nFreeImage)(JNIEnv* env, jobject, jlong _device, jlong image, jlong memory) {
    VkDevice device = (VkDevice)_device;
...
jni_linux_vkextmemory(void, nFreeVkInstance)(JNIEnv* env, jobject, jlong _instance) {
